...
boost() {
  ...
  for PY_VERSION in ${PY_VERSION_LIST[@]} ; do
    ...
    if { ${SHOULD_BUILD_BOOST} ; } ; then
      ...
      start=$(date +%s)
      wget -q "https://archives.boost.io/release/${BOOST_VERSION}/source/${BOOST_PACKAGE_BASENAME}.tar.gz" -O ${BOOST_PACKAGE_BASENAME}.tar.gz || true
      end=$(date +%s)
      ...
      if [ ! -f "${BOOST_PACKAGE_BASENAME}.tar.gz" ] || [[ $(sha256sum "${BOOST_PACKAGE_BASENAME}.tar.gz" | cut -d " " -f 1 ) != "${BOOST_SHA256SUM}" ]] ; then
        ...
        start=$(date +%s)
        wget -q "https://carla-releases.s3.us-east-005.backblazeb2.com/Backup/${BOOST_PACKAGE_BASENAME}.tar.gz" -O ${BOOST_PACKAGE_BASENAME}.tar.gz || true
        end=$(date +%s)
      ...
      ./b2 toolset="${BOOST_TOOLSET}" cxxflags="${BOOST_CFLAGS}" -d0 --prefix="../${BOOST_BASENAME}-install" -j ${CARLA_BUILD_CONCURRENCY} stage release 
      ./b2 toolset="${BOOST_TOOLSET}" cxxflags="${BOOST_CFLAGS}" -d0 --prefix="../${BOOST_BASENAME}-install" -j ${CARLA_BUILD_CONCURRENCY} install
...
libxerces() {
  ...
  if [[ -d ${XERCESC_INSTALL_DIR} &&  -d ${XERCESC_INSTALL_SERVER_DIR} ]] ; then
    ...
    if [[ ! -f "${XERCESC_BASENAME}.tar.gz" ]] ; then
      ...
      start=$(date +%s)
      wget -q "https://carla-releases.s3.us-east-005.backblazeb2.com/Backup/${XERCESC_BASENAME}.tar.gz" || true
      end=$(date +%s)
...
# ==============================================================================
# -- Install the dependencies in parallel --------------------------------------
# ==============================================================================
eigen_and_chrono() {
  eigen |& tee eigen.txt
  if ${USE_CHRONO} ; then
...
# For some unknown reason sqlite and proj have to be outside a function.
# TODO: Figure out why and add them to the list above
